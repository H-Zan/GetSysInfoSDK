package com.admai.getsysinfosdk.Permission;
 ...
 * </ul>
 * 麦子互动的以下权限需要在Android6.0上被允许，麦子互动广告sdk才能正常工作，开发者需要在调用麦子互动的任何代码之前，提前让用户允许权限
 * <ul>
 * <li>必须申请的权限
 * <ul>
...
public class PermissionHelper {
	...
	/**
	 * 麦子互动 Android SDK 所需要向用户申请的权限列表
	 */
	private PermissionModel[] mPermissionModels = new PermissionModel[] {
			...
			// 定位属于麦子互动可选申请权限
			new PermissionModel("位置信息", Manifest.permission.ACCESS_FINE_LOCATION, "我们需要您允许我们获取的你位置，方便我们为你提供精准的服务", ACCESS_FINE_LOCATION_CODE)
	};
		...
		try {
			for (final PermissionModel model : mPermissionModels) {
				if (PackageManager.PERMISSION_GRANTED != ContextCompat
					                                         .checkSelfPermission(mActivity, model.permission)) {
					ActivityCompat
						.requestPermissions(mActivity, new String[] {model.permission }, model.requestCode);
					return;
				}
			}
